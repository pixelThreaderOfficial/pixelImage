    ...
    const handleScale = async () => {
        ...
        await performSmartSave({
            ...
            processFn: async (outDir) => {
                const results: api.ProcessedImage[] = [];
                ...
                for (const path of inputFiles) {
                    ...
                    const targetWidth = Math.round(meta.width * (scale / 100));
                    const targetHeight = Math.round(meta.height * (scale / 100));

                    try {
                        setProgress({
                            current: processedCount + 1,
                            total: inputFiles.length,
                            percentage: (processedCount / inputFiles.length) * 100,
                            current_file: path.split(/[/\\]/).pop() || "",
                            status: `Scaling ${path.split(/[/\\]/).pop()}...`
                        });

                        const bulkRes = await api.processImages({
                            image_paths: [path],
                        ...
                        if (bulkRes.processed.length > 0) {
                            results.push(bulkRes.processed[0]);
                        }
                        processedCount++;
                    } catch (e) {
                        console.error(`Failed to scale ${path}:`, e);
                    }
                }
                ...
                return {
                    processed: results,
                    session_id: uuidv4(),
                    total_input_size: results.reduce((acc, r) => acc + r.input_size, 0),
                    total_output_size: results.reduce((acc, r) => acc + r.output_size, 0),
                    total_time_ms: 0,
                };
            },
            onComplete: (response) => {
        ...
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">Smart Scaler</h1>
                        <p className="text-muted-foreground">
                            Batch scale images by percentage with high-quality resampling
                        </p>
                    </div>
                </div>
            ...
            <div className="grid gap-8 lg:grid-cols-3">
                <Card className="lg:col-span-2">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <div>
                            <CardTitle>Files to Scale</CardTitle>
                            <CardDescription>
                                {inputFiles.length > 0
                                    ? `${inputFiles.length} files selected`
                                    : "Add images to start scaling"}
                            </CardDescription>
                        </div>
                    ...
                    <CardContent className="pt-6">
                        {inputFiles.length > 0 ? (
                                                ...
                                                <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                                                    ...
                                                    {result ? (
                                                        <p className="text-[11px] font-bold text-green-600">
                                                            Scaled to {result.width}x{result.height}
                                                        </p>
                                                    ) : meta && (
                            ...
                            >
                                ...
                                <h3 className="text-xl font-semibold">Select Images to Scale</h3>
                                <p className="text-sm text-muted-foreground mt-2 max-w-xs mx-auto">
                                    Scaled images maintain their aspect ratio. Supports multiple files at once.
                                </p>
                            </div>
                        )}
                    </CardContent>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Adjust scale percentage</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-8">
                            <div className="space-y-6">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-2">
                                        <Label className="text-base text-wrap overflow-hidden">Scale Factor</Label>
                                        {scale > 100 ? (
                                    ...
                                    <span className="text-sm font-mono px-2 py-0.5 rounded bg-primary/10 text-primary">{scale}%</span>
                                </div>
                                <Slider
                                    value={[scale]}
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-[11px] text-muted-foreground">
                                    {saveMode === 'photos'
                                        ? "Individual scaled images will be saved to your output folder."
                                        : "All scaled images will be bundled into a single ZIP archive."}
                                </p>
                            </div>
                        </CardContent>
                        <CardFooter>
                            ...
                            >
                                {isProcessing ? (
                                    ...
                                    <>
                                        ...
                                        Scale {inputFiles.length} {inputFiles.length === 1 ? 'Image' : 'Images'}
                                    </>
                                )}
                            </Button>
                        </CardFooter>
                        ...
                        <Card className="bg-primary/5 border-primary/20">
                            <CardHeader className="pb-3">
                                <div className="flex justify-between items-end">
                                    <CardTitle className="text-sm font-medium">Batch Scaling</CardTitle>
                        ...
                        <Card className="bg-green-500/10 border-green-500/20 overflow-hidden">
                            <CardHeader className="pb-4">
                                <div className="flex items-center gap-2 text-green-700">
                                    ...
                                    <CardTitle>Scaling Complete</CardTitle>
                                </div>
                            </CardHeader>
                            <CardContent className="p-4 bg-background/50 text-center">
                                <p className="text-sm font-medium">Processed {results.length} images at {scale}% scale</p>
                            </CardContent>
                        </Card>
                    )}
                </div>
